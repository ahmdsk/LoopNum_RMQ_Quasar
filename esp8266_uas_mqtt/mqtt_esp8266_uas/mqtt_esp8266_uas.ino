...
const char* password = "Angguncantik";
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(), "/iot:kelas_edp", "kelas_edp")) {
      ...
      client.publish("routing_edp_ahmad", "[CLIENT AHMAD] Init");
      // ... and resubscribe
...
void loop() {
  ...
  for (int i = 1; i <= 20; i++) {
    snprintf(msg, MSG_BUFFER_SIZE, "[CLIENT AHMAD] Perulangan Ke: %ld", i);
    Serial.println(msg);
    client.publish("routing_edp_ahmad", msg);
    delay(1000);
  }
}
